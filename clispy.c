...
lval* lval_call (lenv* e, lval* f, lval* a) {
  ...
  while (a->count) {
    ...
    /* Special case to deal with '&' (variable length args) */
    if (strcmp(sym->sym, "&") == 0) {
  ...
  if (f->formals->count == 0) { /* all formals are bound */
    f->env->par = e;
    return builtin_eval(f->env, lval_add(lval_sexpr(), lval_copy(f->body)));
  } else {
    return lval_copy(f); /* partially evaluated function */
  }
}
...
lval* builtin_ord (lenv* e, lval* a, char* op) {
  ...
  int r;
  int first = a->cell[0]->num;
  int second = a->cell[1]->num;
...
lval* builtin_var (lenv* e, lval* a, char* func) {
  ...
  for (int i=0; i < syms->count; i++) {
    LASSERT(a, (syms->cell[i]->type == LVAL_SYM),
	    "Function '%s' cannot define non-symbol for argument %i. "
	    "Got %s, Expected %s.", func, i,
  ...
  for (int i=0; i < syms->count; i++) {
    /* if 'def' define in globally. If '=' define it locally */
